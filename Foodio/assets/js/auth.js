...
/* ========================================================================== */
/* SECURITY: PASSWORD HASHING SYSTEM                                          */
/* ========================================================================== */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm) {
        ...
        ['email', 'password'].forEach(id => {
            ...
            if (el) {
                el.addEventListener('input', () =>
                    Utils.clearFieldError(id, id + '-err')
                );
            }
        });
        ...
        loginForm.addEventListener('submit', async (e) => {
            ...
            if (!email) {
                Utils.showFieldError('email', 'email-err', 'Email is required');
                isValid = false;
            }

            if (!password) {
                Utils.showFieldError('password', 'password-err', 'Password is required');
                isValid = false;
            }
            ...
            try {
                ...
                // Static accounts
                if (email === "admin@foodio.com" && password === statics.ADMIN)
                    return saveSessionAndRedirect('Admin', email, 'admin');

                if (email === "resto@foodio.com" && password === statics.RESTAURANT)
                    return saveSessionAndRedirect('Restaurant', email, 'restaurant');

                if (email === "delivery@foodio.com" && password === statics.DELIVERY)
                    return saveSessionAndRedirect('Delivery', email, 'delivery');
        ...
        if (verifyEmailForm) {
            verifyEmailForm.addEventListener('submit', (e) => {
                ...
                if (!email || !Utils.isValidEmail(email)) {
                    Utils.showFieldError('resetEmail', 'resetEmail-err', 'Invalid email');
                    return;
                }
    ...
    if (registerForm) {
        ...
        fields.forEach(id => {
            ...
            if (el) {
                el.addEventListener('input', () =>
                    Utils.clearFieldError(
                        id,
                        id === 'username' ? 'name-err'
                        : id === 'regEmail' ? 'email-err'
                        : id === 'phone' ? 'phone-err'
                        : id === 'regPassword' ? 'pass-err'
                        : 'confirm-err'
                    )
                );
            }
        ...
        registerForm.addEventListener('submit', async (e) => {
            ...
            if (!username) {
                Utils.showFieldError('username', 'name-err', 'Full Name is required');
                isFormValid = false;
            }

            if (!email) {
                Utils.showFieldError('regEmail', 'email-err', 'Email is required');
                isFormValid = false;
            } else if (!Utils.isValidEmail(email)) {
                Utils.showFieldError('regEmail', 'email-err', 'Invalid format');
                isFormValid = false;
            }

            if (!phone) {
                Utils.showFieldError('phone', 'phone-err', 'Phone is required');
                isFormValid = false;
            } else if (!Utils.isValidPhone(phone)) {
                Utils.showFieldError('phone', 'phone-err', 'Invalid phone');
                isFormValid = false;
            }

            if (!password) {
                Utils.showFieldError('regPassword', 'pass-err', 'Password is required');
                isFormValid = false;
            } else if (!Utils.isStrongPassword(password)) {
                Utils.showFieldError('regPassword', 'pass-err', 'Weak password');
                isFormValid = false;
            }

            if (!confirm) {
                Utils.showFieldError('confirmPassword', 'confirm-err', 'Confirm password');
                isFormValid = false;
            } else if (password !== confirm) {
                Utils.showFieldError('confirmPassword', 'confirm-err', 'No match');
                isFormValid = false;
            }
